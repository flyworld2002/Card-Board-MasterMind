-- Migration 016: support previewing/creating a listing that doesn't
-- exist on eBay yet (docs/plans/listing-pricing-system.md, PLANNING
-- session 14, item 1 "Create a genuinely new eBay listing").
--
-- Part 1: resolve_listing_prices() can't resolve a template whose
-- listing_id is still NULL — it looks the template up via
-- `WHERE listing_id = p_listing_id`, and NULL = anything is never true
-- in SQL. Adds an optional p_template_id uuid parameter: look up by
-- template_id when provided, by (platform, listing_id) otherwise —
-- fully backward compatible (both existing positional 2-arg SQL calls
-- and the web app's named-JSON RPC calls are unaffected; RETURNS TABLE
-- shape is unchanged).
--
-- Also fixes a real correctness bug this surfaced: the "how much is
-- already claimed by OTHER listings" subtraction compares
-- `pl2.listing_id <> p_listing_id` — when p_listing_id is NULL (the new
-- template_id-based call), SQL's three-valued NULL logic makes that
-- comparison NULL for every row, which silently excludes ALL other
-- listings from the subtraction (available_qty would show the FULL
-- unsold pool, over-promising quantity on a new listing's first push)
-- instead of the correct behavior — a listing that doesn't exist yet
-- can't BE any of those other rows, so every other active listing for
-- the same variant should count as already claimed. Fixed with
-- `(p_listing_id IS NULL OR pl2.listing_id <> p_listing_id)`.
--
-- Part 2: listing_templates gains the listing-level metadata
-- AddFixedPriceItem needs that nothing has ever had to capture before
-- (every prior push only ever revised a listing that already existed
-- with this metadata set once, manually, on eBay's side). Confirmed
-- with Fei: metadata comes from either cloning an existing listing
-- (via GetItem) or manual entry — both write the same columns, source
-- just records which. Fei's account uses eBay Business Policies, so
-- shipping/return/payment are profile ID references, not raw blocks.
-- listing_id itself already supports NULL correctly (existing partial
-- unique index idx_listing_templates_platform_listing_id only applies
-- WHERE listing_id IS NOT NULL) — no change needed there.

ALTER TABLE listing_templates
  ADD COLUMN IF NOT EXISTS source text,
  ADD COLUMN IF NOT EXISTS cloned_from_listing_id text,
...
ALTER TABLE listing_templates
  ADD CONSTRAINT chk_listing_templates_source
  CHECK (source IS NULL OR source IN ('cloned', 'manual'));

DROP FUNCTION IF EXISTS resolve_listing_prices(text, text);

CREATE OR REPLACE FUNCTION resolve_listing_prices(
  p_platform text,
