-- Migration 036: resolve_listing_prices() honors listing_templates.
-- display_sort for real (docs/plans/listing-pricing-system.md build log,
-- 2026-08-16). Previously hardcoded `ORDER BY set_name, card_number_numeric`
-- regardless of what display_sort said — 'alpha'/'release_date' were
-- selectable in the web UI but did nothing. Extends this one function
-- (rather than adding a parallel one) since it's the single source of
-- truth every caller (web roster table, create_listing(),
-- _do_promotions()) already goes through.
--
-- 5 modes now real: 'card_number' (default, unchanged: grouped by set
-- then number), 'number' (number only, ignores set), 'alpha' (by card
-- name), 'release_date' (by each card's set release_year, then set_name
-- since multiple sets can share a release_year — confirmed live: "Mega
-- Evolution" and "Phantasmal Flames" both 2025 — then number),
-- 'rarity' (by rarities.sort_order, migration 035, then number).
--
-- The dynamic ORDER BY uses conditional CASE keys, not dynamic SQL/
-- EXECUTE — every mode's key columns evaluate to NULL (a no-op in a
-- multi-column ORDER BY) except whichever one v_display_sort actually
-- matches, with the original set_name/card_number_numeric kept as the
-- unconditional final tiebreaker/fallback for 'card_number' and any
-- unrecognized value.

BEGIN;

CREATE OR REPLACE FUNCTION resolve_listing_prices(p_platform text, p_listing_id text DEFAULT NULL::text, p_template_id uuid DEFAULT NULL::uuid)
