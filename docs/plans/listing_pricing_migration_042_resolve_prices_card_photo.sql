-- Migration 042: resolve_listing_prices() also returns card_photo_id,
-- so create_listing() (the only push call site that reads its picture
-- data via this RPC rather than a raw listing_card_assignments query —
-- see importer/card_photos.py, migration 041) can resolve the new
-- per-copy photo groups too, not just the legacy eps_picture_url.
--
-- Same DROP FUNCTION + CREATE requirement confirmed in migration 040 —
-- Postgres refuses to let CREATE OR REPLACE change a RETURNS TABLE
-- function's output columns at all, even appended at the end.
