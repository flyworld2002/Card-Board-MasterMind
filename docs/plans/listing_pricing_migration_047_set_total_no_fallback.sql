-- Migration 047: {set_total} uses base_set_number only, no total_cards fallback
--
-- Migrations 039/044/046 had {set_total} prefer base_set_number (the
-- real printed denominator) and fall back to total_cards (the full
-- catalog count including secret rares) when base_set_number was
-- unset. Fei's explicit call (2026-08-18): drop the fallback entirely
-- -- total_cards is the wrong number for this token conceptually (it's
-- never what's actually printed on a card), so silently falling back
-- to it risks a wrong-but-plausible-looking denominator on some future
-- set instead of an obviously-blank one that prompts fixing the real
-- data. Safe right now: every set backing a live listing already has
-- base_set_number set (confirmed via audit right before this request),
-- so this is a zero-behavior-change simplification today, not a
-- regression -- it only changes behavior for a FUTURE set that goes
-- live without base_set_number backfilled first.
--
-- total_cards is untouched everywhere else in this function (still
-- used for the {prefix}-only auto-detection branch).
...
BEGIN
  ...
  -- {set_total} = base_set_number only, no total_cards fallback -- see
  -- header note. Renders blank (via COALESCE below) if unset, a
  -- visible signal to go backfill it rather than a silently-wrong
  -- full-catalog count.
  v_set_total_text := CASE WHEN v_base_set_number ~ '^[0-9]+$' THEN v_base_set_number END;
