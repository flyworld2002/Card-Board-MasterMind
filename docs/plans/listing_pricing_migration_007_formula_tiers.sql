-- Migration 007: formula-based tiers in pricing_profile_tiers.
--
-- Adds nullable `multiplier` and `plus` columns so a tier can compute
-- `market_price * multiplier + plus` instead of a flat `list_price`.
-- Lets one profile mix flat tiers for low brackets with an open-ended
-- formula tier for "anything over $2" (min_market=2.00, max_market=NULL,
-- list_price=NULL, multiplier=2, plus=1) — replicates the old
-- ultra_rare_rule formula behavior, scoped per-tier instead of
-- profile-wide. Purely additive: list_price becomes nullable but every
-- existing row keeps its flat price untouched, and resolve_listing_prices()
-- falls back to the old flat-price path whenever list_price is set.
--
-- A tier must have list_price OR multiplier set (CHECK constraint) — a
-- tier with neither would silently resolve to nothing in the RPC's
-- COALESCE and fall through to the default formula, masking a data-entry
-- mistake instead of failing loudly at insert time.

ALTER TABLE pricing_profile_tiers ALTER COLUMN list_price DROP NOT NULL;
ALTER TABLE pricing_profile_tiers ADD COLUMN multiplier numeric;
ALTER TABLE pricing_profile_tiers ADD COLUMN plus numeric;
ALTER TABLE pricing_profile_tiers ADD CONSTRAINT chk_tier_price_or_formula
  CHECK (list_price IS NOT NULL OR multiplier IS NOT NULL);
...
BEGIN
  ...
  priced AS (
    SELECT
      ...
      m.market_price,
      l.manual_price,
      -- price before the floor: pin -> tier (flat list_price, or
      -- market*multiplier+plus when the matched tier is formula-based) ->
      -- default formula when no tier matches at all
      COALESCE(
        (
          SELECT CASE
