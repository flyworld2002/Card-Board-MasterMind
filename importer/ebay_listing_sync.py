"""
importer/ebay_listing_sync.py — DB -> eBay price recalculation
(--ebay-recalc-prices) plus shared helpers reused by
importer/ebay_pushprices.py (the Listing Pricing System's push command —
see docs/plans/listing-pricing-system.md).

NOTE: this module's own pricing pipeline (card_type_mapping / price_tiers
as a global table / set_pricing_config multiplier+floor) has been
SUPERSEDED by the Listing Pricing System (pricing_profiles +
listing_card_groups + resolve_listing_prices() RPC) — see that plan doc's
PIVOT section. --ebay-recalc-prices is left running against the old
pipeline for now (not explicitly asked to be retired), but new pricing
work should go through the new system, not this one. The push half
(--ebay-push-listings) has been removed entirely — its 250-cap promotion
logic was merged into importer/ebay_pushprices.py's --ebay-pushprices,
since both walked the same listing_card_assignments roster.
